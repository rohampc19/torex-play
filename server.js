...
function sameOrigin(req) { const origin = String(req.headers.origin || ""); return !origin || origin === `http://${req.headers.host}` || origin === `https://${req.headers.host}`; }
...
function normalizeLikeData(data) {
  ...
  (data.news || []).forEach(item => {
    const likes = Array.isArray(item.likes) ? [...new Set(item.likes.filter(Boolean))] : [];
    if (JSON.stringify(likes) !== JSON.stringify(item.likes || [])) { item.likes = likes; changed = true; }
    (item.comments || []).forEach(comment => {
      const commentLikes = Array.isArray(comment.likes) ? [...new Set(comment.likes.filter(Boolean))] : [];
      if (JSON.stringify(commentLikes) !== JSON.stringify(comment.likes || [])) { comment.likes = commentLikes; changed = true; }
    });
  });
  return changed;
}
function loadData() { try { const data = JSON.parse(fs.readFileSync(DATA_FILE, "utf8")); let changed = false; if (!Array.isArray(data.news) || !data.news.length) { data.news = structuredClone(seedNews); changed = true; } data.sessions = Array.isArray(data.sessions) ? data.sessions.filter(session => Date.now() - new Date(session.createdAt).getTime() < 86400000) : []; data.users = Array.isArray(data.users) ? data.users : []; data.users.forEach(user => { if (user.password && !String(user.password).startsWith("pbkdf2$")) { user.password = hashPasswordSync(user.password); changed = true; } }); if (normalizeLikeData(data)) changed = true; if (changed) saveData(data); return data; } catch { const data = structuredClone(initialData); data.news = structuredClone(seedNews); return data; } }
function saveData(data) { fs.writeFileSync(DATA_FILE, JSON.stringify(data, null, 2), "utf8"); }
function reply(res, code, body, extraHeaders = {}) { res.writeHead(code, { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-Content-Type-Options": "nosniff", "Referrer-Policy": "strict-origin-when-cross-origin", "X-Frame-Options": "DENY", "Permissions-Policy": "camera=(), microphone=(), geolocation=()", ...extraHeaders }); res.end(JSON.stringify(body)); }
...
function communityMessageView(data, item) { const sender = data.users.find(user => user.username === item.sender); return { ...item, name: sender?.name || item.sender, avatar: sender?.avatar || item.sender?.[0] || "؟" }; }
...
async function api(req, res, url) {
  ...
  if (!safeRequest(req, res, bucket, bucket === "login" ? 8 : bucket === "register" ? 10 : 180)) return;
  if (["POST", "PUT", "PATCH", "DELETE"].includes(req.method) && !sameOrigin(req)) return reply(res, 403, { error: "مبدأ درخواست معتبر نیست." });
  const data = loadData();
  ...
  if (req.method === "POST" && likeMatch) { const viewer = requireUser(data, req, res); if (!viewer) return; const body = await readBody(req); const item = (data.news || []).find(entry => entry.id === likeMatch[1]); if (!item) return reply(res, 404, { error:"خبر پیدا نشد." }); const username = viewer.username; const uniqueLikes = [...new Set(Array.isArray(item.likes) ? item.likes.filter(Boolean) : [])]; const currentlyLiked = uniqueLikes.includes(username); const desiredLiked = typeof body.liked === "boolean" ? body.liked : !currentlyLiked; item.likes = desiredLiked ? [...uniqueLikes.filter(name => name !== username), username] : uniqueLikes.filter(name => name !== username); saveData(data); return reply(res, 200, { likes:item.likes.length, liked:desiredLiked }); }
  ...
  if (req.method === "POST" && commentLikeMatch) { const viewer = requireUser(data, req, res); if (!viewer) return; const body = await readBody(req); const item = (data.news || []).find(entry => entry.id === commentLikeMatch[1]); const comment = item && (item.comments || []).find(entry => entry.id === commentLikeMatch[2]); if (!comment) return reply(res, 404, { error: "نظر پیدا نشد." }); const uniqueLikes = [...new Set(Array.isArray(comment.likes) ? comment.likes.filter(Boolean) : [])]; const currentlyLiked = uniqueLikes.includes(viewer.username); const desiredLiked = typeof body.liked === "boolean" ? body.liked : !currentlyLiked; comment.likes = desiredLiked ? [...uniqueLikes.filter(name => name !== viewer.username), viewer.username] : uniqueLikes.filter(name => name !== viewer.username); saveData(data); return reply(res, 200, { likes: comment.likes.length, liked: desiredLiked }); }
  ...
  if (req.method === "GET" && url.pathname === "/api/community/messages") { const messages = Array.isArray(data.communityMessages) ? data.communityMessages.slice(-100).map(item => communityMessageView(data, item)) : []; return reply(res, 200, { messages }); }
  if (req.method === "POST" && url.pathname === "/api/community/messages") { const viewer = requireUser(data, req, res); if (!viewer) return; const body = await readBody(req); const text = String(body.text || "").trim(); if (!text || text.length > 500) return reply(res, 400, { error: "متن پیام معتبر نیست." }); data.communityMessages = Array.isArray(data.communityMessages) ? data.communityMessages : []; const message = { id: `cm-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`, sender: viewer.username, text, createdAt: new Date().toISOString() }; data.communityMessages.push(message); data.communityMessages = data.communityMessages.slice(-500); saveData(data); return reply(res, 201, { message: communityMessageView(data, message) }); }
  ...
  if (req.method === "POST" && url.pathname === "/api/login") { const body = await readBody(req); const user = data.users.find(u => u.phone === String(body.phone || "").trim()); const password = String(body.password || ""); if (password.length < 1 || password.length > 128) return reply(res, 401, { error: "شماره موبایل یا رمز عبور نادرست است." }); const valid = user && (String(user.password || "").startsWith("pbkdf2$") ? await verifyPassword(password, user.password) : user.password === password); if (!user || !valid) return reply(res, 401, { error: "شماره موبایل یا رمز عبور نادرست است." }); if (user.status === "pending") return reply(res, 403, { error: "حساب شما هنوز توسط ادمین تایید نشده است." }); if (!String(user.password || "").startsWith("pbkdf2$")) user.password = await hashPassword(password); const value = token(); data.sessions.push({ token: value, userId: user.id, createdAt: new Date().toISOString() }); saveData(data); return reply(res, 200, { user: publicUser(user) }, { "Set-Cookie": `torex_session=${value}; HttpOnly; SameSite=Lax; Path=/; Max-Age=86400${process.env.NODE_ENV === "production" ? "; Secure" : ""}` }); }
  if (req.method === "POST" && url.pathname === "/api/logout") { const header = String(req.headers.authorization || ""); const cookies = String(req.headers.cookie || "").match(/(?:^|; )torex_session=([^;]+)/); const value = header.startsWith("Bearer ") ? header.slice(7) : (cookies ? cookies[1] : ""); data.sessions = (data.sessions || []).filter(item => item.token !== value); saveData(data); return reply(res, 200, { ok: true }, { "Set-Cookie": "torex_session=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0" }); }
