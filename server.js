...
function validateUsername(value) {
  ...
  if (!/^[a-z0-9_]{3,24}$/.test(username)) return { username, error: "نام کاربری باید ۳ تا ۲۴ کاراکتر انگلیسی، عدد یا _ باشد و فاصله نداشته باشد." };
  return { username };
}
function validatePassword(value, repeat) {
  const password = String(value || "");
  if (!password) return "رمز عبور نمی‌تواند خالی باشد.";
  if (password.length < 8) return "رمز عبور باید حداقل 8 کاراکتر باشد.";
  if (password.length > 128) return "رمز عبور نمی‌تواند بیشتر از 128 کاراکتر باشد.";
